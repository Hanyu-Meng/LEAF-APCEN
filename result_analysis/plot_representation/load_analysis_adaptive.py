...
with torch.no_grad():
    ...
    fb_outputs = torch.stack(all_fb_out, dim=1)  
    all_pcen_outputs = torch.stack(all_pcen_outputs, dim=1)  
    all_alpha = torch.stack(all_alpha, dim=1)               
    all_r = torch.stack(all_r, dim=1)
    all_fb_energy = fb_outputs.mean(dim=-1)  
    all_pcen_outputs_energy = all_pcen_outputs.mean(dim=-1)  
    gains = (all_pcen_outputs_energy + 1e-12) / (all_fb_energy + 1e-12) 
    gains = gains.squeeze(0).cpu().numpy()
    gains_db = 20 * np.log10(gains + 1e-12)  

    fb_energy_db = 10 * np.log10(all_fb_energy.squeeze(0).cpu().numpy() + 1e-12)         
    pcen_energy_db = 10 * np.log10(all_pcen_outputs_energy.squeeze(0).cpu().numpy() + 1e-12) 
